...
namespace Dugros_Api.Controllers
{
    ...
    public class PurchaseReportController : ControllerBase
    {
        ...
        public IActionResult GetPurchaseOrderSummary([FromQuery] DateTime? fromDate,[FromQuery] DateTime? toDate, [FromQuery] Guid? statusId,[FromQuery] string? supplierName, [FromQuery] Guid? userId, string? po_no)
        {
            try
            {
        ...
        public IActionResult GetPurchaseIndentDetails([FromQuery] DateTime? fromDate, [FromQuery] DateTime? toDate, [FromQuery] Guid? statusId, [FromQuery] Guid? userId, string? pi_no,string? item_name)
        {
            try
            {
                ...
                using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
                {
                    ...
                    using (var command = new SqlCommand("dbo.sp_Get_PI_Register_Details", connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@user_id", (object)userId ?? DBNull.Value);
                        command.Parameters.AddWithValue("@status_id", (object)statusId ?? DBNull.Value);
                        command.Parameters.AddWithValue("@pi_no", pi_no);
        ...
        public IActionResult GetPurchaseOrderDetails([FromQuery] DateTime? fromDate, [FromQuery] DateTime? toDate, [FromQuery] Guid? statusId, [FromQuery] Guid? userId,string? po_no, string? item_name)
        {
            try
            {
                using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
        ...
        [HttpGet("GetPurchase")]
        public IActionResult GetAllPurchaseIndentDetails([FromQuery] string? status,Guid? purchase_indent_id, Guid? purchase_order_id)
        {
            try
            {
                using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
